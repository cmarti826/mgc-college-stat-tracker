...
export default function CourseSetupPage() {
  ...
  const totalPar = useMemo(
    () => holes.reduce((a, h) => a + Number(h.par || 0), 0),
    [holes]
  )
  const totalYards = useMemo(
    () => holes.reduce((a, h) => a + Number(h.yards || 0), 0),
    [holes]
  )

  const setAllPar = (value: number) => {
    const next = holes.map((h) => ({ ...h, par: value }))
    setHoles(next)
  }
  const setFrontBackPar = (front: number, back: number) => {
    const next = holes.map((h) => ({
      ...h,
      par: h.number <= 9 ? front : back,
    }))
    setHoles(next)
  }
  ...
  async function handleSave() {
    try {
      ...
      // 2) Create tee set (par from holes)
      const teePayload = {
        course_id: courseId,
        name: teeName.trim(),
        par: totalPar,
      ...
      const { error: tErr } = await supabase.from('tee_sets').insert(teePayload)
      if (tErr) throw tErr
      ...
      const holesPayload = holes.map((h) => ({
        ...
        // handicap omitted here (optional in schema)
      }))
      ...
      // Go back to /courses list or directly to the course holes page if you prefer
      router.push('/courses')
    } catch (e: any) {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ marginTop: 8, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        <button
          onClick={() => {
            // Common-ish par-72 pattern
            const pattern: number[] = [4, 4, 3, 4, 5, 4, 3, 5, 4, 4, 4, 3, 5, 4, 4, 3, 5, 4]
            ...
            {holes.map((h, idx) => (
              <tr key={h.number}>
                ...
                <td style={td}>
                  <input
                    type="number"
                    value={h.yards as any}
                    onChange={(e) => {
                      const val = e.target.value
                      const next = [...holes]
                      next[idx].yards = val === '' ? '' : Number(val)
                      setHoles(next)
                    }}
                    style={{ width: 120 }}
