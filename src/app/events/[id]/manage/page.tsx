...
type EventRow = {
  ...
  course_id: string|null; course_tee_id: string|null
}
...
export default function ManageEventPage() {
  ...
  const loadAll = async () => {
    ...
    // courses list (optional selector)
    ...
    const { data: en } = await supabase
      .from('event_entries').select('player_id').eq('event_id', eventId)
    const ens = (en ?? []) as Entry[]
    setEntries(ens)

    // get players for entries
    const pids = ens.map(x => x.player_id)
    if (pids.length) {
      const { data: ps } = await supabase
        .from('players').select('id,display_name').in('id', pids as any)
    ...
    // team roster (add list)
    if (e.team_id) {
      const { data: tr } = await supabase
        .from('team_roster').select('player_id').eq('team_id', e.team_id)
      const trIds = (tr ?? []).map((x:any)=>x.player_id)
      if (trIds.length) {
        const { data: rp } = await supabase
          .from('players').select('id,display_name').in('id', trIds as any).order('display_name')
        setRoster((rp ?? []) as Player[])
        setAddPid(prev => (rp && (rp as Player[]).some(p => p.id===prev) ? prev : ''))
      } else {
        setRoster([]); setAddPid('')
      }
    }
  ...
  // change course -> reload tees
  const onChangeCourse = async (cid: string) => {
    if (!evt) return
  ...
  // save event meta (type, status, dates, course/tee)
  ...
  const removeEntry = async (pid: string) => {
    ...
    try {
      ...
      setErr(e.message ?? 'Failed to remove entry')
    } finally {
      setSaving(false)
    }
  }

  // start round for a player
  const startRound = async (pid: string) => {
    if (!evt) return
    ...
    try {
      ...
      const { data, error } = await supabase.from('rounds').insert(payload as any).select('id').single()
      if (error) throw error
  ...
  const teamName = useMemo(() => (evt ? `Team ${evt.team_id?.slice(0,8)}` : '—'), [evt])
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Link className="rounded border px-3 py-1.5" href={`/events/${eventId}`}>Leaderboard</Link>
      ...
      {evt && (
        <div className="rounded border bg-white p-3">
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
            <Field label="Type">
              <select className="w-full rounded border px-2 py-1" value={evt.type} onChange={e=>setEvt({...evt, type: e.target.value as any})}>
            ...
            <Field label="Status">
              <select className="w-full rounded border px-2 py-1" value={evt.status} onChange={e=>setEvt({...evt, status: e.target.value as any})}>
            ...
            <Field label="Tee">
              <select className="w-full rounded border px-2 py-1" value={evt.course_tee_id ?? ''} onChange={e=>setEvt({...evt!, course_tee_id: e.target.value || null})} disabled={!evt.course_id}>
                <option value="">—</option>
        ...
        <div className="p-3">
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <select className="w-64 rounded border px-2 py-1" value={addPid} onChange={e=>setAddPid(e.target.value)}>
              ...
              {roster
                .filter(r => !entries.some(e => e.player_id===r.id))
              ...
              return (
                <div key={e.player_id} className="flex flex-wrap items-center justify-between gap-3 py-2">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    {active ? (
                      <>
                        <Link className="underline text-[#0033A0]" href={`/rounds/${active.id}`}>Open Round</Link>
      ...
      <div className="rounded border bg-white">
        <div className="flex items-center justify-between border-b px-3 py-2">
          ...
          <Link className="text-sm underline" href={`/events/${eventId}`}>Leaderboard</Link>
        </div>
        <div className="divide-y">
          {rounds.length ? rounds.map(r => (
            <div key={r.id} className="flex items-center justify-between px-3 py-2">
              ...
              <Link href={`/rounds/${r.id}`} className="text-sm underline text-[#0033A0]">Open</Link>
            </div>
          )) : (
